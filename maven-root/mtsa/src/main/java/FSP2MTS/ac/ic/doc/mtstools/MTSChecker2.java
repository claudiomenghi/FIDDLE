...
public class MTSChecker2 {
	...
	public void run(String modelsFile) throws IOException {
		...
		for (Map.Entry<String, Refinement> refinement : this
				.getRefinementNotions().entrySet()) {
			for (Map.Entry<String, MTS<Long, String>> entry1 : MTSs.entrySet()) {
				for (Map.Entry<String, MTS<Long, String>> entry2 : MTSs
						.entrySet()) {
					if (entry1.equals(entry2)) {
						continue;
					}
					try {
						if (refinement.getValue().isARefinement(
								entry1.getValue(), entry2.getValue())) {
							System.out.println(entry2.getKey() + " refines "
									+ entry1.getKey() + " ["
									+ refinement.getKey() + "]");
						}
	...
	private Map<String, MTS<Long, String>> getMTSs(String modelsFile)
			throws IOException {

		CompositeState state = MTSCompiler.getInstance().compileCompositeState(
				"DEFAULT", new File(modelsFile));
	...
	private void loadSemantics() {
		this.getRefinementNotions().put("Weak Alphabet",
				SemanticType.WEAK_ALPHABET.getRefinement());
		this.getRefinementNotions().put("Weak",
				SemanticType.WEAK.getRefinement());
	}
