...
public class UpdatingControllersDefinition extends CompositionExpression {
	...
	public Set<String> generateUpdatingControllableActions(
		...
		Set<String> oldControllableActions = compileSet(oldGoalDef
				.getControllableActionSet());
		Set<String> newControllableActions = compileSet(newGoalDef
				.getControllableActionSet());
	...
	public CompositeState compose(Vector<Value> actuals) {
		CompositionExpression oldController = LTSCompiler.getComposite(this
				.getOldController().toString());
		CompositeState oldC = oldController.compose(null);

		CompositionExpression oldEnvironment = LTSCompiler.getComposite(this
				.getOldEnvironment().toString());
		CompositeState oldE = oldEnvironment.compose(null);

		CompositionExpression newEnvironment = LTSCompiler.getComposite(this
				.getNewEnvironment().toString());
		CompositeState newE = newEnvironment.compose(null);

		// GOAL
		ControllerGoalDefinition oldGoalDef = ControllerGoalDefinition
				.getDefinition(this.getOldGoal());
		ControllerGoalDefinition newGoalDef = ControllerGoalDefinition
				.getDefinition(this.getNewGoal());
		Set<String> controllableSet = this.generateUpdatingControllableActions(
				oldGoalDef, newGoalDef);
		Symbol controllableSetSymbol = LTSCompiler.saveControllableSet(
				controllableSet, this.getName().getValue());
		GRControllerGoal<String> grGoal = UpdatingControllersUtils
				.generateGRUpdateGoal(this, oldGoalDef, newGoalDef,
						controllableSet);
		ControllerGoalDefinition newGoal = UpdatingControllersUtils
				.generateSafetyGoalDef(this, oldGoalDef, newGoalDef,
						controllableSetSymbol, output);

		// MAPPING PROPOSITIONS
		List<Fluent> oldPropositions = UpdatingControllersUtils
				.compileFluents(this.getOldPropositions());
		List<Fluent> newPropositions = UpdatingControllersUtils
				.compileFluents(this.getNewPropositions());

		// DEBUG
		Boolean debugMode = this.debugModelOn();
		List<String> checkTrace = UpdatingControllersUtils
				.compileCheckTraces(this.getCheckTrace());

		UpdatingControllerCompositeState ucce = new UpdatingControllerCompositeState(
				oldC, oldE, newE, newGoal, grGoal, oldPropositions,
				newPropositions, debugMode, checkTrace, name.getValue());
		return ucce;
	}
