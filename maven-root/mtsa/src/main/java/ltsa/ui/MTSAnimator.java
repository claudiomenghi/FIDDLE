 ...
 * 
 * Opposed to the drawings of LTS, this animator can only show changes in either
 * single machines or in the composite machine, however not in submachines of a
 * composite. This would require tracking the current state within every
 * submachine what this class does not do.
 * 
 */
...
public class MTSAnimator implements Animator {
	...
	Dictionary<String, ArrayList<Transition>> transitionsByNameTable; // action
																		// ->
																		// transitions
	Dictionary<Long, ArrayList<Transition>> transitionsByStateTable; // state ->
																		// outgoing
																		// transitions
	...
	// The BitSet which is passed to the AnimWindow (note that indexing starts
	// from 1!)
	...
	public MTSAnimator(CompositeState state, EventManager em) {
		...
		if (!MTSUtils.isMTSRepresentation(state)) {
		...
		if (mtss.size() == 1)
			...
			MTSMultipleComposer<Long, String> composer = new MTSMultipleComposer<Long, String>(
					new CompositionRuleApplier());
	...
	/**
	 * Initialize the animator, generating all dictionaries for easier use in
	 * this class, setting the inital state etc.
	 * 
	 * @param menu
	 *            A list of strings describing a trace
	 * @return
	 */
	...
	public BitSet initialise(Vector menu) {
		...
		for (MTS.TransitionType type : new MTS.TransitionType[] { MTS.TransitionType.REQUIRED,
		...
		if (originalErrorTrace != null && originalErrorTrace.size() != 0
	...
	/**
	 * Change the current state to the next given a transition and make all
	 * necessary modifications including setting the now possible transitions
	 * and notifying the event manager
	 * 
	 * @return
	 */
	...
	private void performTransition(Transition chosenTransition) {
		...
		// The event manager requires an array of the current state of every
		// machine
		// in the MTS plus the composite state and the index of the executed
		// transition
		// Since we're not keeping track of every submachine though, they're all
		// 0
		// except for
		// - the second to last index (the state of the composite)
		// - and the last index which is the transition index (inside the
		// original alphabet)
	...
	/**
	 * Choose a transition from the list of transitions to follow based on the
	 * MTSA behavior; assumes that all transitions are valid, i.e. 'from'
	...
	private Transition chooseTransition(ArrayList<Transition> transitions) {
		...
		// Cases:
		// 1) There is a required transition and at least one maybe, pick the
		// req
		// 2) There is more than one required transition, so choose randomly
		// from those
		// 3) There are only maybe transitions, pick randomly from the maybe
		// ones
		/*
		 * if(req.size() == 1) return req.get(0); else if(req.size() > 1) return
		 * req.get((int)Math.floor(Math.random()*req.size())); else
		 */
	...
	private Transition transitionForIndex(int index) {
		...
		// Can be non-deterministic, if so, choose randomly according to the
		// rules
	...
	/**
	 * Check whether the next part in the trace is still valid in the current
	 * state
	 * 
	 * @return
	 */
	...
	/**
	 * Perform the next step in the trace by using the first step in the trace
	 * to generate the BitSet to return
	 * 
	 * @return
	 */
	...
	private ArrayList<Transition> generateTransitionListFromTrace(Vector<String> trace) {
		...
		for (String step : trace) {
			...
			// If step has ? appended, check if there's a transition that
			// corresponds
			...
			ArrayList<Transition> possibleTransitions = new ArrayList<Transition>(
					Math.max(nameTransitions.size(), stateTransitions.size()));
			...
			// Optimally, there should only be a single transition left now,
			// else there is indeterminism
	...
	/**
	 * Return the correct string object of the action list which matches the
	 * argument
	 * 
	 * @param s
	 * @return
