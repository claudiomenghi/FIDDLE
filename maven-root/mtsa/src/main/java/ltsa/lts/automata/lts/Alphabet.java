...
public class Alphabet {
	...
	String[] myAlpha;
	private LabelledTransitionSystem sm;

	public int maxLevel = 0;

	public Alphabet(LabelledTransitionSystem sm) {
		Preconditions.checkNotNull(sm, "The state cannot be null");
		...
		int tausIndex = 1;
		if (sm.getAlphabet()[Declaration.TAU_MAYBE] != null
				&& sm.getAlphabet()[Declaration.TAU_MAYBE]
...
class PrefixTree {
	...
	PrefixTree[] getSubLists() {
		Vector<PrefixTree> subs = new Vector<>();
		PrefixTree pt = this;
		PrefixTree ptt = list;
		subs.addElement(pt);
		while (ptt != null) {
