...
public class EventState {
	...
	private int next;
	private int machine;
	private EventState list; // used to keep list in event order, TAU first
	private EventState nondet;// used for additional non-deterministic
								// transitions
	private EventState path; // used by analyser & by minimiser

	public EventState(int event, int next) {
		Preconditions.checkArgument(next>=-1, "The id of the state must be grather than or equal to -1");
		this.event = event;
		this.next = next;
	}
	...
	public void setNondet(EventState nondet) {
		this.nondet = nondet;
	}

	public void setPath(EventState path) {
		this.path = path;
	}
	...
	public static EventState replaceNext(EventState anEventState, int oldNext,
			int newNext) {
		if (anEventState == null)
			return null;
		...
		anEventState.list = EventState.replaceNext(anEventState.list, oldNext,
				newNext);

		return anEventState;
	}

	public static EventState addEvent(EventState anEventState, int anEvent,
			int aState) {
			...
			while (q != null) {
				if (q.next > 0 && tauOnly.get(q.next)) {
					...
					for (int i = 1; i < nextS.length; ++i) {
						toAdd = EventStateUtils.add(toAdd, new EventState(
								q.event, nextS[i]));
					}
				}
				q = q.nondet;
		...
		while (p != null) {
			...
			while (q != null) {
				...
				if (q instanceof ProbabilisticEventState) {
					...
					ProbabilisticEventState r = (ProbabilisticEventState) q2.getProbTr();
					while (r != null) {
	...
	public EventState getNondet() {
		return nondet;
	}

	public EventState getPath() {
		return path;
	}
			...
			while (q != null) {
				...
				if (o != null) {
					if (o instanceof Integer) {
						newhead = EventStateUtils.add(newhead, new EventState(
								((Integer) o).intValue(), q.next));
					} else {
						...
						for (Enumeration<Integer> e = v.elements(); e
							...
							newhead = EventStateUtils.add(
									newhead,
									new EventState(((Integer) e.nextElement())
											.intValue(), q.next));
						}
					}
				}
		...
		while (p != null) {
			if (index < p.event) {
				for (int i = index; i < p.event; i++)
					newhead = EventStateUtils.add(newhead, new EventState(i,
							Declaration.ERROR));
			...
			while (q != null) {
				// Avoid adding maybes.
				newhead = EventStateUtils.add(newhead, new EventState(q.event,
						q.next));
				q = q.nondet;
			}
			p = p.list;
	...
	public static EventState removeTransToError(EventState head) {
		...
		while (p != null) {
			if (p.next != Declaration.ERROR)
				newHead = EventStateUtils.add(newHead, new EventState(p.event,
						p.next));
			p = p.list;
		}
		return newHead;
	...
	public static EventState reachableTau(EventState[] states, int k) {
		...
		while (stack != null) {
			int j = stack.next;
			head = EventStateUtils
					.add(head, new EventState(Declaration.TAU, j));
			stack = pop(stack);
