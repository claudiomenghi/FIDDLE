...
////    	Added to make minimisation of CompositeState and LabelledTransitionSystem be consistent.
    	if (CompositeState.reduceFlag) {
         ...
         if (machine.getEndOfSequenceIndex()>=0) {
            ...
            machine.getStates()[es] 
             = EventStateUtils.add(machine.getStates()[es],new LTSTransitionList(machine.getAlphabet().length,es));
         }
         if (machine.hasTau()) {
             initTau();
    ...
    */
    ...
    private LabelledTransitionSystem makeNewMachine() {
        ...
        if (machine.getEndOfSequenceIndex()<0) 
          ...
          m.getStates()[m.getEndOfSequenceIndex()] 
             = LTSTransitionList.remove(m.getStates()[m.getEndOfSequenceIndex()],new LTSTransitionList(m.getAlphabet().length,m.getEndOfSequenceIndex()));
        }
	...
	private LabelledTransitionSystem handleMarkedCompactState(LabelledTransitionSystem m, LabelledTransitionSystem machine, Map<Integer, Integer>
			oldToNew) {
