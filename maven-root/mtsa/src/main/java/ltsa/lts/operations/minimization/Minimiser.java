...
import ltsa.lts.ltscomposition.MarkedCompactState;
...
public class Minimiser {
	...
	private CompactState machTau(CompactState m) {
		...
		for (int i = 0; i < m.states.length; i++)
			m.states[i] = EventStateUtils.add(m.states[i], new EventState(
					Declaration.TAU, i));
		output.out(".");
		return m;
	}
	...
	public CompactState minimise() {
		...
		if (machine.endseq >= 0) {
			...
			machine.states[es] = EventStateUtils.add(machine.states[es],
	...
	private CompactState makeNewMachine() {
		...
		CompactState m = new CompactState(machine.getName());
		...
		for (int i = 0; i < machine.maxStates; i++) {
			...
			EventState tmp = EventStateUtils.renumberStates(machine.states[i],
					oldtonew);
	...
	private CompactState handleMarkedCompactState(CompactState m,
			CompactState machine, Map<Integer, Integer> oldToNew) {
