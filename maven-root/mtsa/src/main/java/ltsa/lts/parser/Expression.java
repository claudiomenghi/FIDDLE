...
public class Expression {
	// TODO EPAVESE get rid of the precision constant and/or make it
	// configurable
	private static MathContext mathCtx = new MathContext(20,
			RoundingMode.HALF_EVEN);
	public static Hashtable<String, Value> constants;

	private static String labelVar(Stack<Symbol> s, Hashtable<String, Value> locals,
	...
	private static Value getVal(Stack<Symbol> s, Hashtable<String, Value> locals,
			Hashtable<String, Value> globals) {
	...
	private static BigDecimal eval(Stack<Symbol> s, Hashtable<String, Value> locals,
			Hashtable<String, Value> globals) {
		Symbol token =  s.pop();
		switch (token.kind) {
		...
		case Symbol.POWER:
			...
			if (right.isNumeric() && left.isNumeric()) {
				return exec_op(token.kind, left.doubleValue(),
						right.doubleValue());
			} else if (token.kind == Symbol.EQUALS
					|| token.kind == Symbol.NOT_EQUAL) {
				if (token.kind == Symbol.EQUALS)
					return left.toString().equals(right.toString()) ? BigDecimal.ONE
							: BigDecimal.ZERO;
				else
					return left.toString().equals(right.toString()) ? BigDecimal.ZERO
							: BigDecimal.ONE;
			} else
		...
		case Symbol.QUESTION:
			...
			return (condition.doubleValue().compareTo(BigDecimal.ZERO) != 0 ? thenBranch
					: elseBranch).doubleValue();
		case Symbol.UNARY_PLUS:
			return eval(s, locals, globals);
		case Symbol.UNARY_MINUS:
			return eval(s, locals, globals).negate();
		case Symbol.PLING:
			return eval(s, locals, globals).compareTo(BigDecimal.ZERO) == 1 ? BigDecimal.ZERO
					: BigDecimal.ONE;
		case Symbol.SINE:
	...
	private static BigDecimal exec_op(int op, BigDecimal left, BigDecimal right) {
		...
		if (op == Symbol.CIRCUMFLEX || op == Symbol.BITWISE_AND
				|| op == Symbol.BITWISE_OR || op == Symbol.SHIFT_LEFT
				|| op == Symbol.SHIFT_RIGHT || op == Symbol.AND
				|| op == Symbol.OR) {
			if (!LTSUtils.isInteger(left) || !LTSUtils.isInteger(right)) {
				Diagnostics.fatal("ERROR: Operation " + op
		...
		case Symbol.SHIFT_LEFT:
			return new BigDecimal(left.intValue() << right.intValue());
		case Symbol.LESS_THAN:
			return left.compareTo(right) == -1 ? BigDecimal.ONE
					: BigDecimal.ZERO;
		case Symbol.LESS_THAN_EQUAL:
			return left.compareTo(right) != 1 ? BigDecimal.ONE
					: BigDecimal.ZERO;
		...
		case Symbol.GREATER_THAN_EQUAL:
			return left.compareTo(right) != -1 ? BigDecimal.ONE
					: BigDecimal.ZERO;
		case Symbol.EQUALS:
			return left.compareTo(right) == 0 ? BigDecimal.ONE
					: BigDecimal.ZERO;
		case Symbol.NOT_EQUAL:
			return left.compareTo(right) != 0 ? BigDecimal.ONE
					: BigDecimal.ZERO;
		case Symbol.AND:
			return left.compareTo(BigDecimal.ZERO) != 0
					&& right.compareTo(BigDecimal.ZERO) != 0 ? BigDecimal.ONE
					: BigDecimal.ZERO;
		case Symbol.OR:
			return left.compareTo(BigDecimal.ZERO) != 0
					|| right.compareTo(BigDecimal.ZERO) != 0 ? BigDecimal.ONE
