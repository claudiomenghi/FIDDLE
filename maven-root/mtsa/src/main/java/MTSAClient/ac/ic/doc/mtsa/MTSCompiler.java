...
public class MTSCompiler {
	...
	public MTS<Long, String> compileMTS(String modelName, File inputFile)
			throws Exception {
		CompositeState compositeState = compileCompositeState(modelName,
				inputFile);
		return AutomataToMTSConverter.getInstance().convert(
				compositeState.getComposition());
	}

	public MTS<Long, String> compileMTS(String modelName, String inputString)
			throws Exception {
		CompositeState compositeState = compileCompositeState(modelName,
				inputString);
		return AutomataToMTSConverter.getInstance().convert(
	...
	public CompositeState compileCompositeState(String modelName, LTSInput input)
			throws IOException {
		StandardOutput output = new StandardOutput();
		return compileComposite(modelName, input, output);
	}

	public CompositeState compileCompositeState(String modelName,
			File inputFile, LTSOutput output) throws IOException {
		LTSInput input = new FileInput(inputFile);
		return compileComposite(modelName, input, output);
	}
	...
	public CompositeState compileComposite(String modelName, LTSInput input,
			LTSOutput output) throws IOException {
	...
	public LTSCompiler getCompiler(LTSInput input) throws IOException {
		LTSCompiler compiler = new LTSCompiler(input, new StandardOutput(),
