...
import controller.game.gr.GRUtils;
...
import controller.game.model.RankBasedGameSolver;
		...
		for (S state : winningStates) {
			for (int guaranteeId = 1 ; guaranteeId <= this.getBGRGoal().livenessSize() ; guaranteeId++) {
				...
				for (S succ : this.getGame().getUncontrollableSuccessors(state)) {
					Validate.isTrue(this.isBetterThan(source,new StrategyState<S,Integer>(succ, nextGuaranteeId),isGoalState, isBuchiState), "State: " + succ + " must have a better rank than state: " + state);
				...
				for (S succ : this.getGame().getControllableSuccessors(state)) {
					if (this.isBetterThan(source,new StrategyState<S,Integer>(succ, nextGuaranteeId), isGoalState, isBuchiState)) {
